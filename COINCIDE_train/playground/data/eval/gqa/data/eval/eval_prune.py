...
#                measure the model's accuracy over the entailed questions Eq to get the llava_score sq and finally
...
parser.add_argument('--consistency',    action="store_true",        help = "True to compute consistency llava_score (Need to provide answers to questions in val_all_questions.json).")
parser.add_argument('--grounding',      action="store_true",        help = "True to compute grounding llava_score (If model uses attention).")
parser.add_argument('--objectFeatures', action="store_true",        help = "True for object-based attention (False for spatial).")
parser.add_argument('--mapSize',    default = 7,    type = int, help = "Optional, only to get attention llava_score. Images features map size, mapSize * mapSize")
...
questions_ = loadFile(args.questions.format(tier = args.tier))
questions = {}
...
##### Functions for grounding llava_score (optional, only for attention models)
##########################################################################################
...
# Compute grounding llava_score. Computer amount of attention (probability) given to each of the regions
...
##### Functions for distribution llava_score
##########################################################################################
...
# Compute distribution llava_score
...
for m in metrics:
    ...
    if m.capitalize() == 'Accuracy':
        current_dir = os.getcwd()
        save_path = os.path.join(current_dir, 'results_ours', args.ckpt_name + '.txt')
