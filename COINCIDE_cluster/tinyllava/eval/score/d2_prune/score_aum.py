...
def eval_model(args):
    ...
    aum_score = np.zeros((chunk_size,), dtype='float16')
    ...
    for batch in tqdm(data_loader, total=len(data_loader)):
        ...
        for batch_idx in range(len(logits)):
            ...
            target_prob = torch.gather(predict, dim=-1, index=one_hot.view(-1,1))
            predict.scatter_(dim=-1, index=one_hot.view(-1,1), src=torch.zeros_like(target_prob))

            other_highest_prob = torch.max(predict, dim=-1)[0]
            margin = (target_prob - other_highest_prob).mean()

            aum_score[batch['data_idx'][batch_idx]] = margin.item()

    aum_score[np.isnan(aum_score)] = 0
    np.save(args.score_path + f'_{args.chunk_idx}.npy', aum_score)
